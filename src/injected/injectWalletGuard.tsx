...
const addWalletGuardProxy = (provider: any) => {
    ...
    apply: async (target: any, thisArg: any, args: any[]) => {
      ...
      if (request.method === 'eth_sendTransaction') {
        ...
        if (response === Response.Reject) {
          ...
          throw ethErrors.provider.userRejectedRequest('Vibranium Shield Tx Signature: User denied transaction signature.');
        }
        ...
        if (response === Response.Reject) {
          ...
          throw ethErrors.provider.userRejectedRequest(
            'Vibranium Shield Message Signature: User denied message signature.'
          );
        }
          ...
          .then((response: any) => {
            if (response === Response.Reject) {
              ...
              const error = ethErrors.provider.userRejectedRequest(
                'Vibranium Shield Tx Signature: User denied transaction signature.'
              );
              const response = {
                id: request?.id,
                jsonrpc: '2.0',
  ...
  // if provider and Vibranium Shield is not in provider
